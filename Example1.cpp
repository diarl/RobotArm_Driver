...
int main()
{
    ...
    Ret=LS.WriteString("Nailed IT!!");                                             // Send the command on the serial port
...
while(1){
    ...
    Ret=LS.ReadString(Buffer,'\r',128,5000);                                // Read a maximum of 128 characters with a timeout of 5 seconds
                                                                            // The final character of the string must be a line feed ('\n')
    if (Ret>0)                                                              // If a string has been read from, print the string
        printf (" String read from serial port : %s \n \r",Buffer);
    else
        printf (" TimeOut reached. No data received ! \r \n");                   // If not, print a message.
